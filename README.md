...
Parallel to the architecture work, we are building a high-quality medical reasoning dataset by upgrading `32K` examples from [MedReason](https://huggingface.co/datasets/UCSC-VLAA/MedReason) — originally generated by GPT-4o using knowledge graph paths from PrimeKG.

The upgrade pipeline sends each example through GPT-5.4 / Gemini with a domain-aware system prompt (v4.1) that:
- deepens mechanistic reasoning (pathophysiology, molecular pathways, receptor kinetics)
- applies GRADE-style evidence assessment when citing guidelines
- conditionally applies a longevity/preventive lens only when clinically appropriate
- self-verifies by re-answering from the upgraded reasoning alone

The hard constraint: **the ground-truth answer is never changed**. Only the reasoning path is refined.
...
### Experiment 6: Medical Reasoning Distillation

| Metric | Value |
|---|---|
| MedQA accuracy (distilled Qwen3.5-0.8B) | **36%** |
| MedQA accuracy (vanilla Qwen3.5-0.8B) | 20% |
| Distilled examples | 100 (scaling to 32K) |
| Distillation accuracy | 95% answer preservation |
| MedReason v4.1 upgrades | 141 examples, 92.2% pass rate |
| Self-verification pass rate | 88% (11 verify failures caught and discarded) |

The distilled model nearly doubles the base model's MedQA score with only 100 training examples. The MedReason upgrade pipeline has processed 141 examples across all 9 source datasets with answer preservation verified by an independent model call.
